...
archer::archer(int lv_in, string name_in)
{
	...
	HPmax =135+7*(LV - 1);		// HP increases 8 points per level
	HP = HPmax;
	MPmax = 80+ 2 * (LV - 1);		// MP increases 2 points per level
	MP = MPmax;
	AP = 30 + 5 * (LV - 1);			// AP increases 4 points per level
	DP = 20 + 3 * (LV - 1);			// DP increases 4 points per level
	speed = 28 + 3 * (LV - 1);		// speed increases 2 points per level
...
void archer::isLevelUp()
{
	if (EXP >= LV*LV * 75)
	{
		...
		cout << "HP improved 7 points to " << HPmax << endl;
...
bool archer::attack(player &p)
{
	...
	{
		HPtemp = (int)((1.0*AP / p.DP)*hit*AP * 5 / (rand() % 4 + 10));		// opponent's HP decrement calculated based their AP/DP, and uncertain chance
	...
	// Normal attack
	HPtemp = (int)((1.0*AP / p.DP)*hit*AP * 5 / (rand() % 4 + 10));
